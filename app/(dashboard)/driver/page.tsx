...
export default function DriverDashboard() {
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      const firstGroup = Array.isArray(json.data) ? (json.data[0] ?? null) : null;
      setGroup(firstGroup);
    } catch {
      toast.error("Тооцоонийлэх мэдээлэл уншихад алдаа гарлаа");
    } finally {
      setIsLoading(false);
    }
  }, [date]);
  ...
  return (
    <div>
      <Header title="Тооцоо нийлэх" subtitle="Жолоочийн хүргэлтийн тайлан" />
      ...
      <div className="p-5 space-y-4">
        ...
        {isLoading ? (
          ...
          <Card padding="none">
            <div className="py-14 text-center text-sm text-slate-400">
              Сонгосон өдөр хүргэсэн захиалга алга байна
            </div>
          </Card>
        ) : (
          <Card padding="none">
            <div className="px-5 py-4 border-b border-slate-100 bg-slate-50 rounded-t-2xl">
              <p className="font-semibold text-slate-800">{group.driverName}</p>
            </div>
              ...
              <table className="w-full text-sm">
                ...
                <tfoot>
                  <tr className="bg-slate-50 border-t border-slate-200">
                    <td colSpan={6} className="px-4 py-3 text-sm font-semibold text-slate-600">Тайлангийн мэдээлэл</td>
                    <td className="px-4 py-3 text-right font-bold text-slate-700">{formatPrice(group.deliveredAmount)}</td>
                    <td className="px-4 py-3 text-right font-bold text-violet-600">{formatPrice(group.driverFee)}</td>
                    <td className="px-4 py-3 text-right font-bold text-slate-800">{formatPrice(group.companyPayout)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
        ...
        <p className="text-xs text-slate-400">
          * Хүргэсэн захиалга бүрт жолооч 6,000₮ бодогдоно. &nbsp;|&nbsp;
          Тооцоо орсон үед тушаах дүн 0 болно. &nbsp;|&nbsp;
          Бусад хүргэсэн захиалгад тушаах дүн = захиалгын дүн − 6,000₮
        </p>
      </div>
    </div>
  );
