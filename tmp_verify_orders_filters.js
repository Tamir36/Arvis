...
const ALL_ORDER_STATUSES = ['BLANK', 'PENDING', 'CONFIRMED', 'PACKED', 'SHIPPED', 'DELIVERED', 'CANCELLED', 'RETURNED'];
...
async function countForDay(statuses, dateStr) {
  ...
  const where = {};
  const andFilters = [];
  const fromDateValue = parseDateStart(dateStr);
  const toDateValue = toDayEnd(parseDateStart(dateStr));
  const dateRange = { gte: fromDateValue, lte: toDateValue };
  ...
  const deliveredLatestLogs = await prisma.orderAuditLog.groupBy({ by:['orderId'], where:{ action:'STATUS_CHANGED', newValue:'DELIVERED' }, _max:{ createdAt:true } });
  ...
  const cancelledLatestLogs = await prisma.orderAuditLog.groupBy({ by:['orderId'], where:{ action:'STATUS_CHANGED', newValue:'CANCELLED' }, _max:{ createdAt:true } });
  ...
  const returnedLatestLogs = await prisma.orderAuditLog.groupBy({ by:['orderId'], where:{ action:'STATUS_CHANGED', newValue:'RETURNED' }, _max:{ createdAt:true } });
  ...
  if (!includesToday) {
    if (hasStatusFilter) {
      const noTodayStatuses = requestedStatuses.filter((status) => !ROLLOVER_STATUSES.includes(status));
      if (noTodayStatuses.length > 0) andFilters.push({ status:{ in: noTodayStatuses } });
      else andFilters.push({ id:{ in: [] } });
    } else {
      andFilters.push({ status:{ notIn:[...ROLLOVER_STATUSES] } });
    }
  }
